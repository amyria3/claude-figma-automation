...
- **Tabellen auf einheitliche/volle Breite bringen:** GitHub-Markdown-Tabellen passen sich dem Inhalt an; feste Breiten gibt es nicht. Workaround: unsichtbarer Platzhalter im Header der breiten Spalte — `| Komponente <img width="700" height="1" alt=""> | ... |`. Das `img` ohne `src` rendert leer, erzwingt aber die Mindestbreite der Spalte; alle Tabellen mit gleichem Platzhalter werden gleich breit. Bei zusätzlichen Spalten den Wert entsprechend verkleinern (700px bei 2 Schmalspalten, ~1000px Content-Bereich).
...
- Der CodeMirror-View-Instanzzugriff (`.cm-content.cmView.view.state.doc`) ist auf github.com NICHT verfügbar — Dokumentinhalt immer über die Raw-URL holen, nie aus dem Editor-DOM.
