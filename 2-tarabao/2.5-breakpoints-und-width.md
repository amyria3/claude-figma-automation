...
- **Alle min-/max-Werte hängen an `Lyt scl / Width`.** Keine rohen Werte und keine direkte Bindung an `[twuc]`-Primitive. So schaltet jede Breite mit dem Modus um.
...
- **Figma nimmt an Textebenen keine Bindung an eine Width-Variable an.** Ein max-w an einem Text bindet deshalb direkt ein Primitive (`[twuc]-N`). Das ist nur unkritisch, wenn der Wert in allen Modi gleich ist.
