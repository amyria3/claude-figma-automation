...
3. **Komponenten schalten mit.** NavBar, Nav, Footer und PromoBar haben eine Property `Breakpoint`, der Nutmixer die Property `viewport-range`. Die Property ist an die Text-Variable `viewport-range` gebunden und wählt die Variante, die zum Breitenbereich passt.
...
Wähle die Instanz aus, z. B. den Footer. Im Dev Mode zeigt die Property `Breakpoint` kein festes „md“, sondern die Variable `viewport-range`. Der Footer übernimmt damit die Variante des Modus, in dem er liegt. Im Code braucht er dafür keine Prop.
